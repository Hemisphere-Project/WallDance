...
| **Track P — collapse 3 evidence paths → 2 (GPU-only)** ▶ **ACTIVE (first)** | P | **Staged migration, operator-approved 2026-06-23.** Pre-check ✅ **TRT byte-stable run-to-run** on a fixed engine (both goldens exact). **Stage 1 (the crux):** re-target the tuning/cache/golden harness onto GPU+TRT — today `detect_cache` builds caches *inside* `_process_cpu` (full-frame coords) + replays via `_track_detections`; re-point it to capture **letterbox-space detections at a conf floor + the `_TrackerSpace` + motion gray** from the GPU front-end (`_run_yolo_and_track`, line ~768) and replay through the shared `_post_yolo_chain`. τ stays cheap via the conf-floor + re-apply hook (Phase-2b method; the motion gray is path-independent). **⚠ IRREVERSIBLE checkpoint:** re-baseline the 12 goldens on the TRT cache — byte-cv2 → **engine/driver-locked** (re-baseline on any TRT/driver bump; cross-machine = tolerance only). **Stage 2:** delete the CPU-only surface (`_process_cpu`/`_track_detections`/`_offset_detections`/`_identity_scaled_track`/`_OffsetMotionProxy`, ~450 LOC) + parity test + `--cpu` mode. **Stage 3:** harden the engine pre-show gate. Verify gate (goldens + GPU parity) each stage. Then unblocks known-N (K1). |
...
| Item | Win | Risk |
|------|-----|------|
| **Track P (3→2 GPU collapse)** — ▶ **ACTIVE — see §3.2** | ~450 LOC (CPU front-end + dual coord transforms + parity test) gone; "test what you ship" | Un-deferred 2026-06-23 (sequenced **before** known-N — the search needs the show-path base). The staged plan + the IRREVERSIBLE golden re-baseline checkpoint live in **§3.2 → Track P**. |
