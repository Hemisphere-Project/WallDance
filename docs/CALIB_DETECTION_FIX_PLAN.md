...
| **2** | TOGO-day "flying ghost": a YOLO ghost vanishes but a motion box stays and flies, "often from the ghost spot toward top-left" | **Diagnosed (static).** The frozen-ghost report gate (`_collect_confirmed_tracks`, tracker.py:3140) suppresses a skeleton-stale track (`_frames_since_skeleton > 3`) **only if also near-stationary** (`speed < 0.03·person_height_px ≈ 6 px/frame`). Its assumption "moving ⇒ real dancer" is the loophole: a ghost that picks up velocity (bridge velocity-override / inherited YOLO vel) free-runs through `predict()` and sails through the gate → reported to OSC. **Top-left** because `predict()` clamps only the negative frame edges (tracker.py:415-420, no bottom-right clamp): top-left flyers stop and linger at the corner; any other heading exits off-screen and vanishes. `MAX_VELOCITY = 0.67·box_height` lets a fat (shadow-inflated) ghost box fly ~130 px/frame — the case-1 cross-link. Fix: gate on blob-corroboration not speed (suppress skeleton-stale unless `is_bridged and _bridge_blobless_streak==0`); optionally add the missing bottom-right `predict()` clamp. **Dynamic confirmation in progress** (full slot 9 @ live conf 0.4). | `tracker.py:3140` (gate), `:415` (clamp), `:2917` (vel override) |
...
- **Case 2 — diagnosed, fix pending confirmation.** See §1 row 2. Dynamic reproduction (full slot 9 @ live conf 0.4) running.
- Next: confirm case 2 flyer in the full-slot timeline → fix the report gate → then case 3 (CLAHE/gamma seeds).
