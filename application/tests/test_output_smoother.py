...
def test_two_tracks_independent_and_idset_preserved():
    """The released id set == the reported id set (no suppression — every
    reported track is released L frames late)."""
    L = 4
    sm = OutputSmoother(lag=L)
    released_ids = set()
    for s in range(20):
...
# NOTE: case-2 flying-ghost SUPPRESSION was removed (2026-06).  The output
# smoother now always releases every reported track L frames late (the released
# id set == the reported id set), so there is nothing left to suppression-test.
# See test_two_tracks_independent_and_idset_preserved for the id-set invariant.
