...
- Next: **operator §6.3 full manual pass** over Phase 3 (riskiest phase for UI state-sync; one-tick command latency is the expected behavior delta — during a model load, queued commands now apply after the load instead of mid-load). Then Phase 4 (main-loop isolation) and the queued desktop "Calibrate All" wizard over the new seam.
