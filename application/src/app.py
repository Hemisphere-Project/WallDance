...
class WallDanceApp:
    ...
    def run(self):
        ...
        while self.running and dpg.is_dearpygui_running():
            ...
            self.roi._poll_roi_mouse_interaction()
            ...
            if preview_source_frame is not None:
                ...
                if (src_w, src_h) != self.roi.state.source_size:
                    self.roi._clamp_roi_to_source(src_w, src_h, sync_ui=True)
            ...
            if self.preview_enabled and (self.frame_count % self.preview_stride == 0):
                ...
                if preview_new and preview_input_available:
                    ...
                    if self.settings.roi_enabled and preview_source_frame is not None:
                        ...
                        src_w, src_h = self.roi.state.source_size
                    ...
                    if self.settings.roi_enabled:
                        preview_base = self.roi._compose_roi_preview(display_frame, src_w, src_h)
                    else:
                        preview_base = display_frame if display_frame is not None else preview_source_frame
                    ...
                    if self.settings.roi_enabled:
                        self.roi._draw_roi_mask(preview_frame, src_w, src_h)
                    self.roi._draw_exclusion_overlay(preview_frame, src_w, src_h)
                    ...
                    if self.settings.roi_enabled:
                        self.roi._draw_roi_note(preview_frame, src_w, src_h)
