...
WallDance is built around a **rig → calibrate → monitor** flow so a new venue does not require hand-tuning knobs. The steps below describe the **shipped** flow; the roadmap and forward plan are in [docs/ROADMAP.md](docs/ROADMAP.md) (doc index: [docs/README.md](docs/README.md)). See [docs/archives/UX_PLAN.md](docs/archives/UX_PLAN.md) for the shipped design rationale.
