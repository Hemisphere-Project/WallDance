...
| `app.py` decomposition | Maint. | 🟡 Plan written 2026-06-11 | ~4.8k ln. Staged plan in [DECOMPOSITION_PLAN.md](DECOMPOSITION_PLAN.md): headless core + runtime controllers + command/event seam (Phase 5 = tablet calibration client) |
...
| **Production UX track (U0–U5)** | UX | ✅ Merged to `main` | See [UX_PLAN.md](UX_PLAN.md): expert mode, lighting profiles, calib1 (scene/servo/joint sweep), calib2 (evidence pool/imgsz), sensitivity macro. Numeric rules provisional → re-fit on annotated footage (imgsz target + model choice: §4.2 Phase 2b benchmark) |
...
| P-6 | **imgsz auto-select FPS budget** (bug #12e) | ✅ **Done 2026-06-11** (§4.2 Phase 2 ⑤c): `select_imgsz` models fps ∝ imgsz⁻² from the runs' measured fps and rejects presets predicted under `AUTOCAL2_FPS_BUDGET`; a height-target miss is an explicit rig advisory, never a silent fallback. Remaining extension: the cost model is **model-agnostic** — per-model fps factors (measured once per rig at engine-build time) land with the §4.2 Phase 2b benchmark | Prevents a silent worst-case regression | None — it was a missing guard |
