...
| 3 | **Signal fixes**: bug #9 (letterbox pad @ scale=1) + bug #4 (frame-diff stale pair cap), each validated on replay + new transform test cases | Correctness of the primary ghost/relay signal on the quiet Starvis2 scenes we are building toward |
...
| 4 | **Confirmed real** → fix | `motion_detector.feed_preprocessed` | `_curr_raw`/`_prev_raw` advance only when the *global* peak diff > 8. On a clean static stretch the pair freezes and `frame_diff_blob_in_bbox` keeps reporting the **last motion event's diff indefinitely** — and frame-diff is now the primary ghost-killer *and* relay signal. Sensor noise self-heals it on dirty footage (why it never bit on residence1-solo); the quiet Starvis2 + even-IR scenes we're building toward are exactly the at-risk regime. Fix: frames-since-advance cap → report zero past N frames (keeps the slow-motion accumulate behavior under the cap). Validate on replay. |
| 5 | ✅ Covered (P3 0) | ROI→letterbox→unscale transform chain | Now under [test_transforms.py](../application/tests/test_transforms.py) (round-trip + crossval/exclusion transform). **Gap found 2026-06-10:** the `scale == 1.0` + nonzero-pad case is untested — see bug #9. |
...
| 9 | Medium (GPU path only) | `pipeline._crossval_motion_filter` + `_exclusion_norm_xy` | `(x − pad)/scale if scale != 1.0 else x` **drops the letterbox pad when `lb_scale == 1.0`** — which happens whenever the ROI long side equals imgsz with nonzero pad on the short axis (e.g. 1280×720 ROI @ imgsz 1280 → pad_y 280). Gate + exclusion then sample the motion mask off by the pad. `_unscale_letterbox` does it correctly (unconditional). Fix the conditionals + add the scale=1+pad case to test_transforms (bug #5 gap). |
| 10 | Structural | `pipeline._run_yolo_and_track` (GPU) vs `_track_detections` (CPU) | The GPU path hand-duplicates the post-YOLO chain (gate → exclusion → cold blobs → tracker → OSC). **All replay/golden/tuning evidence validates the CPU copy; the show runs the GPU copy** — bug #9 lives in exactly this duplication. **Decision (§4.1): fully unify**, parity-test first. |
